...
describe("Token contract", function () {
    ...
    let weth;
    let lpToken;
    let lpTokenAddress = '0xd6d9758b816c3b555489dca76aa84bfa2ca71812';
    let lpTokenContract;
    ...
    async function getPermitSignature(signer, token, spender, value, deadline){
        ...
        console.log("verifyingContract: token.address: ", token.address);
    ...
    async function _addLiquidity(){
        await tokenC.connect(signer[0]).approve(uniswapV2Router.address,TOKEN_A_AMOUNT);
        await tokenD.connect(signer[0]).approve(uniswapV2Router.address,TOKEN_B_AMOUNT);
    ...
    describe("UniswapV2",async()=>{
            ...
            tokenC = await TokenC.connect(signer[0]).deploy(TOKEN_A_AMOUNT);
    
            const TokenB = await ethers.getContractFactory("TokenD");
            tokenD = await TokenB.connect(signer[0]).deploy(TOKEN_A_AMOUNT);
            ...
            describe("Functions",async()=>{
                ...
                it.only("removeLiquidityPermit function", async function () {
                  ...
                  console.log("UniswapV2Router Address: ",uniswapV2Router.address);
                    ...
                    await tokenC.connect(signer[0]).approve(uniswapV2Router.address,TOKEN_A_AMOUNT);
                    await tokenD.connect(signer[0]).approve(uniswapV2Router.address,TOKEN_B_AMOUNT);
                    await uniswapV2Router.connect(signer[0]).addLiquidity(tokenC.address,tokenD.address,TOKEN_A_AMOUNT,TOKEN_B_AMOUNT,1,1,signer[0].address, 1764541741);
                    
                    let bal = (await lpTokenContract.balanceOf(signer[0].address)/1e18);
                    console.log("LpToken balance: ",bal);

                    // let bal2 = 999999999999999999000;
    ...
    //                 expect(iniBalToken).to.be.lessThan(fnlBalToken);
    //                 expect(iniBalEther).to.be.greaterThan(fnlBalEther);
    //                 expect(fnlBalT3).to.equal(10000.300030120392);
    ...
    //                 expect(iniBalToken).to.be.lessThan(fnlBalToken);
    //                 expect(iniBalEther).to.be.greaterThan(fnlBalEther);
    //                 expect(fnlBalT3).to.equal(10000.600030120391);
    //             });
